...
open class ContextMenu: NSObject {
    ...
    private var targetedImage:UIImage?
    ...
    func addTargetedImageView() {
        ...
        self.targetedImage = self.getRenderedImage()
        
        targetedImageView.image = self.targetedImage
    ...
    func prepareViewsForRemoveFromSuperView(with rect: CGPoint) {
        ...
        self.targetedImageView.frame = CGRect(x: rect.x, y: rect.y, width: self.viewTargeted.frame.width, height: tvH)
        self.menuView.alpha = 0
        self.menuView.frame = CGRect(x: rect.x, y: rect.y, width: self.viewTargeted.frame.width, height: 0)
    }
        ...
        if topClippedSpace > 0, bottomClippedSpace > 0 {
            ...
            if diffY > 0 {
                ...
                if (tvY + tvH/2) > mainViewRect.height/2 { //down
                    ...
                    if tvY < MenuConstants.MenuMarginSpace {
                        tvY = MenuConstants.TopMarginSpace
                        mY = mainViewRect.height -  (mH + MenuConstants.MenuMarginSpace + MenuConstants.BottomMarginSpace)
                        tvH =  mainViewRect.height  - mH - MenuConstants.TopMarginSpace - MenuConstants.BottomMarginSpace - tvY
                        targetedImageView.image = cropImage(partHight: tvH)
                    ...
                    tvY = MenuConstants.TopMarginSpace
                    if tvY + tvH  + mH + MenuConstants.MenuMarginSpace + MenuConstants.BottomMarginSpace > mainViewRect.height {
    ...
    func cropImage(partHight:CGFloat) -> UIImage?{
        guard let originalImage = self.targetedImage else{
            return nil
        }
        let originalSize = originalImage.size
        let targetHeight = partHight

        UIGraphicsBeginImageContextWithOptions(CGSize(width: originalSize.width, height: targetHeight), false, 0.0)
                
        let context = UIGraphicsGetCurrentContext()
        context!.clip(to: CGRect(x: 0, y: 0, width: originalSize.width, height: targetHeight))
                
        originalImage.draw(in: CGRect(x: 0, y: 0, width: originalSize.width, height: originalSize.height))
